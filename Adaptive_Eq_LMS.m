...
BER_LMS=zeros(1,length(SNR_dB));%ber vector
mu=0.5;
%--------------------------------------------------------------------------
%iterations----------------------------------------------------------------
for i=1:length(Ks)%pilot count loop
    ...
    for s=1:length(SNR_dB)%snr loop
        ...
        BER_LMS(i,s)=fer/(fr*N);%calculate ber
        [SNR_dB(s) fr fer]%print the current parameters
    end%end snr loop
end%end pilot count loop
w_LMS=w;
save('LMS_Results','BER_LMS','SNR_dB','w_LMS');%save results